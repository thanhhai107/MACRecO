    ...
    def __call__(self, prompt: str, call_type: str = "unknown", *args, **kwargs) -> str:
        ...
        while retry_count <= max_retries:
            try:
                ...
                is_retryable = '429' in error_str or '503' in error_str or 'Resource exhausted' in error_str or 'rate limit' in error_str_lower
                
                if is_retryable and retry_count < max_retries:
                    retry_count += 1
                    wait_time = 1  # 1 second between retries
                    logger.warning(f"Rate limit error calling Gemini model (attempt {retry_count}/{max_retries}): {e}. Retrying in {wait_time} second...")
                    time.sleep(wait_time)
                    continue
                else:
                    if retry_count >= max_retries:
                        logger.error(f"Error calling Gemini model after {max_retries} retries: {e}. Giving up on this call.")
                    else:
